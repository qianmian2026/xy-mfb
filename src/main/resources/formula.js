...
(function(global, factory) {
  ...
  function getMatrixSize(array) {
    ...
    var columnSize = array.reduce(function(maxColSize, row) {
      return Math.max(maxColSize, row.length);
    }, 0);
    return [ rowSize, columnSize ];
  }
  function createMatrix(rowSize, columnSize) {
    var sizeArgs = [ rowSize, columnSize ];
    if (sizeArgs.some(function(size) {
      return !isDefined(size);
    })) return na;
    if (sizeArgs.some(function(size) {
      return size <= 0;
    })) return value;
  ...
  function CHOOSECOLS(array, col_num1) {
    if (!Array.isArray(array)) return value;
    for (var _len = arguments.length, col_nums = new Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
      col_nums[_key - 2] = arguments[_key];
    }
    var colArgs = flatten([ col_num1 ].concat(col_nums));
    var _getMatrixSize = getMatrixSize(array), _getMatrixSize2 = _slicedToArray(_getMatrixSize, 2), rowsSize = _getMatrixSize2[0], colsSize = _getMatrixSize2[1];
    var notInRange = colArgs.some(function(col_num) {
      if (!isDefined(col_num)) return true;
      var absColNum = Math.abs(col_num);
      return !(0 < absColNum && absColNum <= colsSize);
    });
    if (notInRange) return value;
    var targetColNums = colArgs.map(function(col_num) {
      return col_num < 0 ? col_num + colsSize + 1 : col_num;
    });
    var arrayResult = new Array(rowsSize);
    for (var iRow = 0; iRow < rowsSize; iRow++) {
      arrayResult[iRow] = new Array(targetColNums.length);
      for (var iCol = 0; iCol < targetColNums.length; iCol++) {
        var _array$iRow;
        arrayResult[iRow][iCol] = (_array$iRow = array[iRow][targetColNums[iCol] - 1]) !== null && _array$iRow !== void 0 ? _array$iRow : 0;
      }
    }
    return arrayResult;
  }
  function CHOOSEROWS(array, row_num1) {
    if (!Array.isArray(array)) return value;
    for (var _len2 = arguments.length, row_nums = new Array(_len2 > 2 ? _len2 - 2 : 0), _key2 = 2; _key2 < _len2; _key2++) {
      row_nums[_key2 - 2] = arguments[_key2];
    }
    var rowArgs = flatten([ row_num1 ].concat(row_nums));
    var notInRange = rowArgs.some(function(row_num) {
      if (!isDefined(row_num)) return true;
      var absRowNum = Math.abs(row_num);
      return !(0 < absRowNum && absRowNum <= array.length);
    });
    if (notInRange) return value;
    var targetRowNums = rowArgs.map(function(row_num) {
      return row_num < 0 ? row_num + array.length + 1 : row_num;
    });
    return targetRowNums.reduce(function(selectedRows, row_num_selection) {
      var clonedRow = _toConsumableArray(array[--row_num_selection]).map(function(value) {
  ...
  function DROP(array, rows, columns) {
    ...
    var dropResult = [];
    var _getMatrixSize3 = getMatrixSize(array), _getMatrixSize4 = _slicedToArray(_getMatrixSize3, 2), rowsSize = _getMatrixSize4[0], colsSize = _getMatrixSize4[1];
    var iRowStart = 0, iRowEnd = rowsSize;
    if (typeof rows === "number") {
      var absRowsArg = Math.abs(rows);
    ...
    var iColStart = 0, iColEnd = colsSize;
    if (typeof columns === "number") {
      var absColumnsArg = Math.abs(columns);
    ...
    for (var iRow = iRowStart; iRow < iRowEnd; iRow++) {
      var shallowClonedRow = _toConsumableArray(array[iRow].slice(iColStart, iColEnd).map(function(value) {
  ...
  function EXPAND(array, rows, columns, pad_with) {
    var _getMatrixSize5 = getMatrixSize(array), _getMatrixSize6 = _slicedToArray(_getMatrixSize5, 2), rowSizeOriginal = _getMatrixSize6[0], columnSizeOriginal = _getMatrixSize6[1];
    ...
    var result = createMatrix(rows, columns);
    for (var iRow = 0; iRow < rows; iRow++) {
      for (var iCol = 0; iCol < columns; iCol++) {
        var _array$iRow2;
        var _value = (_array$iRow2 = array[iRow]) === null || _array$iRow2 === void 0 ? void 0 : _array$iRow2[iCol];
        if (!isDefined(_value)) _value = iRow < rowSizeOriginal && iCol < columnSizeOriginal ? 0 : pad_with !== null && pad_with !== void 0 ? pad_with : na;
        result[iRow][iCol] = _value;
      }
    }
    return result;
  }
  ...
  function HSTACK(array) {
    for (var _len3 = arguments.length, otherArrays = new Array(_len3 > 1 ? _len3 - 1 : 0), _key3 = 1; _key3 < _len3; _key3++) {
      otherArrays[_key3 - 1] = arguments[_key3];
    }
    var arrays = [ array ].concat(otherArrays);
    var maxRowCount = 0;
    var totalColCount = 0;
    var colsPerArray = [];
    var _iterator = _createForOfIteratorHelper(arrays), _step;
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done; ) {
        var _currentArray = _step.value;
        var _getMatrixSize7 = getMatrixSize(_currentArray), _getMatrixSize8 = _slicedToArray(_getMatrixSize7, 2), rowSize = _getMatrixSize8[0], colSize = _getMatrixSize8[1];
    ...
    var arrayResult = Array.from({
      ...
      return new Array(totalColCount);
    });
    var iColResult = 0;
    for (var iArr = 0; iArr < arrays.length; iArr++) {
      var currentArray = arrays[iArr];
      var currentArrayColCount = colsPerArray[iArr];
      for (var iCol = 0; iCol < currentArrayColCount; iCol++) {
        for (var iRow = 0; iRow < maxRowCount; iRow++) {
          var _currentArray$iRow$iC, _currentArray$iRow;
          arrayResult[iRow][iColResult] = (_currentArray$iRow$iC = (_currentArray$iRow = currentArray[iRow]) === null || _currentArray$iRow === void 0 ? void 0 : _currentArray$iRow[iCol]) !== null && _currentArray$iRow$iC !== void 0 ? _currentArray$iRow$iC : iRow < currentArray.length ? 0 : na;
        }
        iColResult++;
      }
    }
  ...
  function VSTACK(array) {
    for (var _len4 = arguments.length, otherArrays = new Array(_len4 > 1 ? _len4 - 1 : 0), _key4 = 1; _key4 < _len4; _key4++) {
      otherArrays[_key4 - 1] = arguments[_key4];
    }
    var arrays = [ array ].concat(otherArrays);
    var maxColCount = 0;
    var totalRowCount = 0;
    var _iterator2 = _createForOfIteratorHelper(arrays), _step2;
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
        var _currentArray2 = _step2.value;
        var _getMatrixSize9 = getMatrixSize(_currentArray2), _getMatrixSize0 = _slicedToArray(_getMatrixSize9, 2), rowSize = _getMatrixSize0[0], colSize = _getMatrixSize0[1];
    ...
    var arrayResult = new Array(totalRowCount);
    var currentArray;
    var iRowResult = -1;
    while ((currentArray = arrays.shift()) !== undefined) {
      for (var iRow = 0; iRow < currentArray.length; iRow++) {
        arrayResult[++iRowResult] = new Array(maxColCount);
        for (var iCol = 0; iCol < maxColCount; iCol++) {
          var _currentArray$iRow$iC2;
          arrayResult[iRowResult][iCol] = (_currentArray$iRow$iC2 = currentArray[iRow][iCol]) !== null && _currentArray$iRow$iC2 !== void 0 ? _currentArray$iRow$iC2 : iCol < currentArray[iRow].length ? 0 : na;
        }
      }
    }
    return arrayResult;
  ...
  function MID(text, start_num, num_chars) {
    ...
    var numArgs = [ start_num, num_chars ].map(function(num) {
      return parseNumber(num);
    });
    var validNumArgs = numArgs.every(function(numArg) {
    ...
    var _numArgs = _slicedToArray(numArgs, 2);
    start_num = _numArgs[0];
    num_chars = _numArgs[1];
  ...
  function roundBase(number, num_digits, roundFn) {
    ...
    var pair = (val + "e" + num_digits).split("e");
    var shifted = roundFn(pair[0] + "e" + pair[1]);
    pair = (shifted + "e" + -num_digits).split("e");
    var rounded = Number(pair[0] + "e" + pair[1]);
    return rounded * sign;
  }
  ...
  function IMSUM() {
    ...
    var numberSum = 0;
    var imaginarySum = 0;
    var _iterator3 = _createForOfIteratorHelper(args), _step3;
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done; ) {
        var arg = _step3.value;
        var realPart = +IMREAL(arg);
        var imaginaryPart = +IMAGINARY(arg);
  ...
  exports.CHOOSECOLS = CHOOSECOLS;
  exports.CHOOSEROWS = CHOOSEROWS;
  exports.CLEAN = CLEAN;
  exports.CODE = CODE;
